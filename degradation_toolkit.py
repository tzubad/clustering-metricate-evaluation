...
class ClusteringDegrader:
    ...
    def __init__(self, csv_path: str, cluster_col: str = 'cluster_id', 
                 embedding_col: str = 'reduced_embedding', random_seed: int = 42):
        """
    ...
    def _parse_embeddings(self):
        ...
        self.df_original['_parsed_embedding'] = self.df_original[self.embedding_col].apply(parse_embedding)
    ...
    def _compute_cluster_stats(self):
        ...
        for cluster_id in valid_clusters[self.cluster_col].unique():
            ...
            cluster_embeddings = np.stack(valid_clusters.loc[cluster_mask, '_parsed_embedding'].values)
    ...
    def remove_tight_clusters(self, n_clusters: int = 1, 
                               criteria: Literal['tightest', 'smallest', 'largest'] = 'tightest') -> pd.DataFrame:
        """
        ...
        if criteria == 'tightest':
            sorted_clusters = sorted(self.cluster_stats.keys(), 
                                    key=lambda x: self.cluster_stats[x]['tightness_score'], 
                                    reverse=True)
        elif criteria == 'smallest':
            sorted_clusters = sorted(self.cluster_stats.keys(), 
                                    key=lambda x: self.cluster_stats[x]['size'])
    ...
    def label_swap(self, fraction: float = 0.1, 
                   swap_type: Literal['random', 'neighboring', 'distant'] = 'random') -> pd.DataFrame:
        """
    ...
    def merge_clusters(self, n_merges: int = 1, 
                       merge_type: Literal['random', 'nearest', 'farthest'] = 'nearest') -> pd.DataFrame:
        """
    ...
    def split_clusters(self, n_splits: int = 1, 
                       split_type: Literal['random', 'largest', 'loosest'] = 'largest') -> pd.DataFrame:
        """
        ...
        if split_type == 'random':
            clusters_to_split = np.random.choice(all_clusters, size=min(n_splits, len(all_clusters)), replace=False)
        elif split_type == 'largest':
            clusters_to_split = sorted(all_clusters, 
                                       key=lambda x: self.cluster_stats[x]['size'], 
                                       reverse=True)[:n_splits]
        elif split_type == 'loosest':
            clusters_to_split = sorted(all_clusters, 
                                       key=lambda x: self.cluster_stats[x]['tightness_score'])[:n_splits]
    ...
    def generate_degradation_suite(self, output_dir: str = 'degraded_datasets', 
                                   levels: List[float] = [0.05, 0.1, 0.25, 0.5]):
        """
...
if __name__ == "__main__":
    ...
    print(f"   Clusters: {degrader.df_original['cluster_id'].nunique()} -> {df['cluster_id'].nunique()}")
